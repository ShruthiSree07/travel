...
export default function TipSplitterPage() {
  ...
  const error = useMemo(() => {
    ...
    if (!bill.trim() || !Number.isFinite(b) || b < 0) return "Enter a bill amount of 0 or more.";
    if (!tipPercent.trim() || !Number.isFinite(t) || t < 0) return "Enter a tip percentage of 0 or more.";
    if (!people.trim() || !Number.isFinite(p) || p <= 0) return "Enter at least 1 person.";
    return null;
    ...
    <CalculatorShell domain={domain!} calculator={calculator!}>
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
        <div className="space-y-5">
          <Field label="Bill amount" suffix="USD" value={bill} onChange={(e) => setBill(e.target.value)} className={style.ring} aria-invalid={!!error} />
          <Field
